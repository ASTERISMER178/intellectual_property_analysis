...
df = df[df['Patent Type'].isin(['Utility Model Patent','Granted Utility Patent','Design Patent'])]
# 定义三种颜色
colors = ['blue','red']

# 分组并计算均值
grouped = df.groupby('Patent Type')['Patent Value'].mean().reset_index()
# grouped = grouped.sort_values(by='Patent Value', ascending=False)
print(grouped)
# 设置图表
plt.figure(figsize=(8, 6.5))
bars=plt.barh(grouped['Patent Type'], grouped['Patent Value']/10000, color=colors, edgecolor='black')

# 设置横纵坐标标签
plt.xlabel('Average Patent Value(Ten thousand dollars)',fontsize=24)
plt.ylabel('Patent Type',fontsize=24)
plt.xticks(fontsize=24)
plt.yticks([])
# 显示图表
plt.grid(True, axis='x', linestyle='--', alpha=0.6)
plt.title('Average Patent Value by Patent Type',fontsize=24)

for bar, label in zip(bars, grouped['Patent Type']):
    bar.set_label(label)

# 显示图例
plt.legend(loc='upper right',bbox_to_anchor=(1.0, 1.0),fontsize=24)
plt.tight_layout()
plt.savefig('../picture/type_mean_analysis.png')
plt.show()